...
class MyPaydateCalculator implements PaydateCalculatorInterface {
     ...
     * then applies simple logic to increase or decrease the date
     *
            ...
            if($this->isHoliday($date)){
                while(true){
                    ...
                    if($this->isValidPaydate($newPotentialDate)){
                        $possibleDates[$position] = $newPotentialDate;
                        break;
                    }
    ...
    public function isValidPaydate(string $date): bool{
        if($this->isToday($date) || $this->isWeekend($date) || $this->isHoliday($date)){
            return false;
        }
        return true;
    }
